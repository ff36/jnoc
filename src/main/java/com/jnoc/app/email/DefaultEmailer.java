...
public class DefaultEmailer implements Emailer {
	...
	public void send(Email email) {
		 ...
		 */
		if (!email.getVariables().containsKey(JNOC.EmailVariableKey.TIME)) {
			SimpleDateFormat simpleDateFormat = new SimpleDateFormat(
					JNOC.TemporalFormat.TIME_FORMAT.getValue());
			String time = simpleDateFormat.format(Calendar.getInstance()
					.getTime());
			email.getVariables().put(JNOC.EmailVariableKey.TIME, time);
		}
		if (!email.getVariables().containsKey(JNOC.EmailVariableKey.DATE)) {
			SimpleDateFormat simpleDateFormat = new SimpleDateFormat(
					JNOC.TemporalFormat.DATE_FORMAT.getValue());
			String date = simpleDateFormat.format(Calendar.getInstance()
					.getTime());
			email.getVariables().put(JNOC.EmailVariableKey.DATE, date);
		}
		...
		if (email.getTemplate().getLinkPath() != null) {
			...
			email.getVariables().put(JNOC.EmailVariableKey.LINK, link);
		}
		...
		for (Entry<JNOC.EmailVariableKey, String> entry : email.getVariables()
				.entrySet()) {
