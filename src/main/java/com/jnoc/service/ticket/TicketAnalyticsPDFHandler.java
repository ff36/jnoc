...
import com.jnoc.service.ticket.TicketAnalytics.TicketDigest;
	...
	private void prepareData() {
		if(tickets!=null){
	        ...
	        for(int i=0; i<tickets.size(); i++){
				...
				if(JNOC.TicketStatus.CLOSED.equals(ticket.getStatus())){
                	statusClose++;
                ...
                if(JNOC.TicketSeverity.S1.equals(ticket.getSeverity()))
                	s1++;
                else if(JNOC.TicketSeverity.S2.equals(ticket.getSeverity())){
                	s2++;
                }else if(JNOC.TicketSeverity.S3.equals(ticket.getSeverity()))
                	s3++;
				
                if(JNOC.TicketTopic.CONNECTION_FAILURE.equals(ticket.getTopic())){
                	connectionFailure++;
                }else if(JNOC.TicketTopic.EQUIPMENT_FAILURE.equals(ticket.getTopic()))
                	equipmentFailure++;
                else if(JNOC.TicketTopic.GENERAL.equals(ticket.getTopic()))
                	general++;
                else if(JNOC.TicketTopic.INFORMATIONAL.equals(ticket.getTopic()))
                	informational++;
                else if(JNOC.TicketTopic.SIGNAL_SOURCE_FAILURE.equals(ticket.getTopic()))
                	signalSourceFailure ++;
                else if(JNOC.TicketTopic.TOTAL_SYSTEM_FAILURE.equals(ticket.getTopic()))
                	totalSysFailure ++;
			...
			status.add(new ReportTicket(JNOC.TicketStatus.OPEN.toString(), statusOpen));
	        status.add(new ReportTicket(JNOC.TicketStatus.CLOSED.toString(), statusClose));
	        ...
	        topics.add(new ReportTicket(JNOC.TicketTopic.CONNECTION_FAILURE.getLabel(), connectionFailure, "0"));
	        topics.add(new ReportTicket(JNOC.TicketTopic.EQUIPMENT_FAILURE.getLabel(), equipmentFailure, "0"));
	        topics.add(new ReportTicket(JNOC.TicketTopic.GENERAL.getLabel(), general, "0"));
	        topics.add(new ReportTicket(JNOC.TicketTopic.INFORMATIONAL.getLabel(), informational, "0"));
	        topics.add(new ReportTicket(JNOC.TicketTopic.SIGNAL_SOURCE_FAILURE.getLabel(), signalSourceFailure, "0"));
	        topics.add(new ReportTicket(JNOC.TicketTopic.TOTAL_SYSTEM_FAILURE.getLabel(), totalSysFailure, "0"));
