...
import co.ff36.jnoc.per.project.JNOC.KeyType;
...
public class DefaultStorageManager implements StorageManager {
	...
	public String keyGenerator(KeyType keyType, String id) {
		...
		case USER_PROFILE_IMAGE:
			...
			key = JNOC.StorageDirectory.USERS.getValue() + "/"
		...
		case COMPANY_LOGO:
			...
			key = JNOC.StorageDirectory.COMPANIES.getValue() + "/"
					+ company.getS3id() + "/"
		...
		case COMPANY_DIRECTORY:
			...
			break;
		case TEMPORARY_FILE:
			key = JNOC.StorageDirectory.CORE.getValue() + "/"
					+ JNOC.StorageDirectory.TEMPORARY.getValue() + "/" + id;
			break;
		case TICKET_ATTACHEMENT:
			key = JNOC.StorageDirectory.TICKETS.getValue() + "/"
					+ JNOC.StorageDirectory.TICKET_ATTACHMENTS.getValue() + "/"
					+ id;
			break;
		case EMAIL_BLACKLIST:
			key = JNOC.StorageDirectory.CORE.getValue() + "/"
					+ JNOC.StorageDirectory.CORE_EMAIL.getValue() + "/"
					+ JNOC.StorageFile.EMAIL_BLACKLIST.getValue();
			break;
		}
		return key;
	}
